...
@interface CPHealthKitManager ()
//HKHealthStore类提供用于访问和存储用户健康数据的界面。
@property (nonatomic, strong) HKHealthStore *healthStore;
@end
@implementation CPHealthKitManager

#pragma mark - 健康单例
+ (instancetype)sharedInstance {
    static CPHealthKitManager *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[CPHealthKitManager alloc] init];
    });
    return instance;
}
...
- (void)getStepCount:(void(^)(NSString *stepValue, NSError *error))completion{
     ...
     */
    HKSampleQuery*query = [[HKSampleQuery alloc] initWithSampleType:stepType predicate:[CPHealthKitManager getStepPredicateForSample] limit:HKObjectQueryNoLimit sortDescriptors:@[start,end] resultsHandler:^(HKSampleQuery * _Nonnull query, NSArray<__kindof HKSample *> * _Nullable results, NSError * _Nullable error) {
        if(error){
            completion(0,error);
        }else{
...
- (void)getSleepCount:(void(^)(NSString *sleepValue, NSError *error))completion{
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:sleepType predicate:[CPHealthKitManager getSleepPredicateForSample] limit:HKObjectQueryNoLimit sortDescriptors:@[sortDescriptor] resultsHandler:^(HKSampleQuery * _Nonnull query, NSArray<__kindof HKSample *> * _Nullable results, NSError * _Nullable error) {
        if (error) {
            NSLog(@"=======%@", error.domain);
        }else{
            NSLog(@"resultCount = %ld result = %@",results.count,results);
...
- (void)getLast3DaysStepCount:(void(^)(NSMutableArray *stepValue, NSError *error))completion beforeDays:(NSInteger)beforeDays result:(NSMutableArray *)stepResults {
    ...
    /*
     ...
     @param         resultsHandler  结束后返回结果
     */
    HKSampleQuery*query = [[HKSampleQuery alloc] initWithSampleType:stepType predicate:[CPHealthKitManager getLastDayStepOf:beforeDays/*getSleepPredicateForSample*/] limit:HKObjectQueryNoLimit sortDescriptors:@[start,end] resultsHandler:^(HKSampleQuery * _Nonnull query, NSArray<__kindof HKSample *> * _Nullable results, NSError * _Nullable error) {
        if(error){
            completion(0,error);
        }else{
            NSLog(@"resultCount = %ld result = %@",results.count,results);
            ...
            NSDate *now = [NSDate dateWithTimeIntervalSinceNow:-beforeDays * 24 * 60 * 60];//[NSDate date];
            NSDateFormatter * formatter = [[NSDateFormatter alloc] init];
            [formatter setDateFormat:@"yyyy-MM-dd"];
            NSString *startFormatValue = [formatter stringFromDate:now];
            NSDictionary *result = @{
                                     @"date" : startFormatValue,
                                     @"step" : @(totleSteps)
                                     };
            
            [stepResults addObject:result];
            
            [self getLast3DaysStepCount:completion beforeDays:beforeDays + 1 result:stepResults];
        }
    }];
...
+ (NSPredicate *)getLastDayStepOf:(NSInteger)day {
    
    NSDate *now = [NSDate dateWithTimeIntervalSinceNow:-day * 24 * 60 * 60];//[NSDate date];
    NSDateFormatter * formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyyMMdd"];
    NSString *startFormatValue = [NSString stringWithFormat:@"%@000000",[formatter stringFromDate:now]];
    NSString *endFormatValue = [NSString stringWithFormat:@"%@235959",[formatter stringFromDate:now]];
